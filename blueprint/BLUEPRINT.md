...
- [x] **Process 1 Restructure: Persistent `:core` Process Optimization**: Streamlined `:core` to contain strictly essential background components (`HandleService`, edge gesture detection, `NetSpeedManager` + continuous foreground notification with zero-allocation cached `DynamicSpeedIconGenerator`, `CallStateReceiver` / `CallRecorderManager`, and `BootReceiver`). Implemented `onTrimMemory()` forwarding and deferred UI cache initializations away from `:core` startup.
